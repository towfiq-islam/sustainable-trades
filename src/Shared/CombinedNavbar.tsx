...
const CombinedNavbar = ({ dynamicPage }: any) => {
  ...
  const favoritesHref = isLoggedIn
    ? user?.role === "customer"
  ...
  const notificationHref = isLoggedIn
    ? user?.membership?.membership_type === "pro"
      ...
      <Container>
        <div className="flex justify-between items-center">
                ...
                return (
                  ...
                  >
                    ...
                    >
                      {item?.label}
                      <span
                        className={`transition-transform text-sm duration-300 ${
          ...
          <div className="flex gap-3 2xl:gap-5 items-center">
            {/* Message (logged-in only) */}
            {isLoggedIn && (
              <Link
                href={messagesHref}
                aria-label="Messages"
                className="cursor-pointer text-accent-white hover:opacity-80 transition-opacity duration-200"
              >
                <MessageSvg />
              </Link>
            )}

            {/* Notification (vendors only) */}
